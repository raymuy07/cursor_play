...
from scripts.db_utils import JobsDB, get_db_connection, JOBS_DB
...
class ManualEditor:
    ...
    def setup_departments_tab(self):
        ...
        self.dept_synonyms_scrollbar = ttk.Scrollbar(synonyms_frame, orient=tk.VERTICAL, command=self.dept_synonyms_listbox.yview)
        ...
        ttk.Entry(synonym_buttons_frame, textvariable=self.dept_new_synonym_var, width=30).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(synonym_buttons_frame, text="Add", command=self.add_department_synonym).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(synonym_buttons_frame, text="Remove Selected", command=self.remove_department_synonym).pack(side=tk.LEFT)
    ...
    def setup_locations_tab(self):
        ...
        self.loc_synonyms_scrollbar = ttk.Scrollbar(synonyms_frame, orient=tk.VERTICAL, command=self.loc_synonyms_listbox.yview)
        ...
        ttk.Entry(synonym_buttons_frame, textvariable=self.loc_new_synonym_var, width=30).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(synonym_buttons_frame, text="Add", command=self.add_location_synonym).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(synonym_buttons_frame, text="Remove Selected", command=self.remove_location_synonym).pack(side=tk.LEFT)
    ...
    def refresh_departments(self):
        ...
        departments = self.jobs_db.get_all_departments()
    ...
    def load_department_synonyms(self, dept_id: int):
        ...
        with get_db_connection(JOBS_DB) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT synonym FROM department_synonyms WHERE department_id = ? ORDER BY synonym",
                (dept_id,)
            )
    ...
    def load_location_synonyms(self, loc_id: int):
        ...
        with get_db_connection(JOBS_DB) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT synonym FROM location_synonyms WHERE location_id = ? ORDER BY synonym",
                (loc_id,)
            )
            for row in cursor.fetchall():
        ...
        try:
            with get_db_connection(JOBS_DB) as conn:
                ...
                if self.current_dept_id is None:
                    ...
                    cursor.execute(
                        "INSERT INTO departments (canonical_name, category) VALUES (?, ?)",
    ...
    def add_location_synonym(self):
        ...
        try:
            with get_db_connection(JOBS_DB) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO location_synonyms (synonym, location_id) VALUES (?, ?)",
    ...
    def delete_department(self):
        ...
        try:
            with get_db_connection(JOBS_DB) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT COUNT(*) FROM jobs WHERE department_id = ?",
    ...
    def delete_location(self):
        ...
        try:
            with get_db_connection(JOBS_DB) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT COUNT(*) FROM jobs WHERE location_id = ?",
                    (self.current_loc_id,)
                )
                job_count = cursor.fetchone()[0]
        except Exception as e:

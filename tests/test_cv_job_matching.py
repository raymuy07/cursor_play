...
try:
    ...
    print("Please run: pip install sentence-transformers numpy")
    sys.exit(1)
...
class JobMatcher:
    ...
    def _load_cv_embeddings(self, path: str) -> Dict:
        ...
        self.logger.info(f"CV embeddings loaded successfully")
        self.logger.info(f"  - Model: {data['model_name']}")
        self.logger.info(f"  - Embedding dimension: {data['embedding_dim']}")
        self.logger.info(f"  - CV text length: {data['text_length']} characters")
        self.logger.info(f"  - Generated: {data['timestamp']}")
    ...
    def load_jobs(self, jobs_file: str, max_jobs: int = 30) -> List[Dict]:
        ...
        """
        self.logger.info(f"Loading jobs from: {jobs_file}")
        ...
        self.logger.info(f"Loaded {len(jobs)} jobs (out of {len(all_jobs)} total)")
    ...
    def embed_jobs(self, jobs: List[Dict]) -> List[Dict]:
        ...
        """
        self.logger.info(f"Generating embeddings for {len(jobs)} jobs...")
        ...
        for i, job in enumerate(jobs, 1):
            ...
            if not job_text or not job_text.strip():
                self.logger.warning(f"Job {i} ({job.get('title', 'Unknown')}) has no description text, skipping...")
                continue
            ...
            if i % 10 == 0:
                self.logger.info(f"  Processed {i}/{len(jobs)} jobs...")
        
        self.logger.info(f"Successfully embedded {len(enriched_jobs)} jobs")
    ...
    def score_jobs(self, jobs_with_embeddings: List[Dict]) -> List[Dict]:
        ...
        """
        self.logger.info(f"Calculating similarity scores for {len(jobs_with_embeddings)} jobs...")
        ...
        self.logger.info(f"Job scoring complete")
    ...
    def display_results(self, scored_jobs: List[Dict], top_n: int = 10):
        ...
        """
        self.logger.info("\n" + "=" * 80)
        self.logger.info(f"TOP {top_n} MATCHING JOBS")
        self.logger.info("=" * 80)
        ...
        for i, job in enumerate(scored_jobs[:top_n], 1):
            ...
            self.logger.info(f"  Company: {job.get('company_name', 'N/A')}")
            self.logger.info(f"  Location: {job.get('location', 'N/A')}")
            self.logger.info(f"  Department: {job.get('department', 'N/A')}")
            self.logger.info(f"  Experience Level: {job.get('experience_level', 'N/A')}")
            self.logger.info(f"  Workplace Type: {job.get('workplace_type', 'N/A')}")
            self.logger.info(f"  Description Length: {job.get('text_length', 0)} characters")
        ...
        self.logger.info(f"Total jobs scored: {len(scored_jobs)}")
        self.logger.info(f"Average similarity: {np.mean(scores) * 100:.2f}%")
        self.logger.info(f"Highest similarity: {np.max(scores) * 100:.2f}%")
        self.logger.info(f"Lowest similarity: {np.min(scores) * 100:.2f}%")
        self.logger.info(f"Standard deviation: {np.std(scores) * 100:.2f}%")
...
def main():
    ...
    try:
        ...
        if not jobs_with_embeddings:
            logger.error("No jobs were successfully embedded. Exiting.")
            sys.exit(1)
